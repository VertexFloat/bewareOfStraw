-- Author: VertexFloat
-- Date: 04.09.2022
-- Version: Farming Simulator 22, 1.0.0.8
-- Copyright (C): VertexFloat, All Rights Reserved
-- BewareOfStraw main file
            ...
            if lastArea > 0 then
                if workArea.chopperAreaIndex ~= nil and spec.workAreaParameters.lastChopperValue ~= nil then
                    ...
                    if chopperWorkArea ~= nil then
                        xs, ys, zs = getWorldTranslation(chopperWorkArea.start)
                        xw, yw, zw = getWorldTranslation(chopperWorkArea.width)
                        xh, yh, zh = getWorldTranslation(chopperWorkArea.height)
    ...
    local function addCutterArea(superFunc, self, area, realArea, inputFruitType, outputFillType, strawRatio, strawGroundType, farmId, cutterLoad)
        ...
        if area > 0 and (spec.lastCuttersFruitType == FruitType.UNKNOWN or spec.lastCuttersArea == 0 or spec.lastCuttersOutputFillType == outputFillType) then
            ...
            if getIsCollectingStraw(spec.attachedCutters) == true then
                local randomYield = math.random(1, 16)
